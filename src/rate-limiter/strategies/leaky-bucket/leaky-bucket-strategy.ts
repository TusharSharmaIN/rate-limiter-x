...
import { LeakyBucketConfig } from './leaky-bucket.config';
import { FailOpenCounterService } from '../../../observability/fail-open-counter.service';

@Injectable()
export class LeakyBucketStrategy implements RateLimiterStrategy<LeakyBucketConfig> {
  private readonly logger = new Logger(LeakyBucketStrategy.name);
  ...
  constructor(
    ...
    this.script = fs.readFileSync(
      path.join(__dirname, 'leaky-bucket.lua'),
      'utf-8',
    );
  }
  ...
  async checkLimit(
    ...
    const bucketKey = `leakybucket:${key}`;
    const now = Date.now();
    ...
    try {
      const result = await this.redis.evalScript<[number, number, number]>(
        this.script,
        [bucketKey],
        [config.capacity, config.leakRatePerSec, now],
      );
